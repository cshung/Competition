...
#include "LEET_LONGEST_DUPLICATE_SUBSTRING.h"
#include <map>
#include <iostream>
#include <cassert>
#include <vector>
...
namespace _LEET_LONGEST_DUPLICATE_SUBSTRING
{
    ...
    public:
        string longestDupSubstring(string S)
        {
            ...
            {
                return S.substr(bestEnd - bestDepth, bestDepth);
            }
        }
    private:
        int bestDepth;
        int bestEnd;
        void dfs(suffix_tree& t, suffix_tree_node* n, int depth, int end)
        {
            suffix_tree_node* c = t.get_child(n);
            if (c != nullptr)
            {
                if (depth > bestDepth)
                {
            ...
            while (c != nullptr)
            {
                int begin;
                int end;
                t.get_edge(c, &begin, &end);
                dfs(t, c, depth + (end - begin), end);
                c = t.get_sibling(c);
            }
        }
    };
...
using namespace _LEET_LONGEST_DUPLICATE_SUBSTRING;
