...
impl Solution {
    pub fn find_min_arrow_shots(points: Vec<Vec<i32>>) -> i32 {
        ...
        {
            let point = &points[i];
            events.push(Event { x:point[0], enter:true,  index:i });
            events.push(Event { x:point[1], enter:false, index:i });
            bursted.push(false);
        }
...
{
    let answer = Solution::find_min_arrow_shots(vec![vec![10,16],vec![2,8],vec![1,6],vec![7,12]]);
    println!("{}", answer);
}